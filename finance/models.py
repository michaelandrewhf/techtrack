...
class ServiceAgreement(TimeStampedUUIDModel):
    customer = models.ForeignKey(Customer, on_delete=models.PROTECT, related_name="service_agreements")
    name = models.CharField(max_length=160)
    description = models.TextField(blank=True)
    status = models.CharField(max_length=20, choices=AgreementStatus, default=AgreementStatus.ACTIVE)
    starts_on = models.DateField()
    ends_on = models.DateField(null=True, blank=True)
    billing_frequency = models.CharField(max_length=20, choices=BillingFrequency, default=BillingFrequency.MONTHLY)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(condition=Q(amount__gte=0), name="agreement_amount_non_negative"),
            models.CheckConstraint(condition=Q(billing_day__gte=1) & Q(billing_day__lte=31), name="agreement_billing_day_valid"),
            models.CheckConstraint(condition=Q(ends_on__isnull=True) | Q(ends_on__gte=F("starts_on")), name="agreement_end_after_start"),
        ]
...
class ReceivableQuerySet(models.QuerySet):
    def with_amounts(self):
        ...
        return self.annotate(
            paid_amount=Coalesce(paid, Value(Decimal("0.00")), output_field=DecimalField(max_digits=12, decimal_places=2)),
        ).annotate(
            balance=ExpressionWrapper(F("amount") - F("paid_amount"), output_field=DecimalField(max_digits=12, decimal_places=2))
        )

    def with_list_data(self):
        return self.select_related("customer", "work_order", "service_agreement").with_amounts()
...
class Receivable(TimeStampedUUIDModel):
    ...
    work_order = models.ForeignKey(
        "workorders.WorkOrder", on_delete=models.PROTECT, related_name="receivables", null=True, blank=True
    )
    service_agreement = models.ForeignKey(
        ServiceAgreement, on_delete=models.PROTECT, related_name="receivables", null=True, blank=True
    )
    origin = models.CharField(max_length=20, choices=ReceivableOrigin)
    description = models.CharField(max_length=255)
    reference = models.CharField(max_length=120, blank=True)
    competence = models.DateField(null=True, blank=True, help_text="Primeiro dia do mes de competencia.")
    issued_at = models.DateField(default=timezone.localdate)
    due_date = models.DateField()
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    status = models.CharField(max_length=20, choices=ReceivableStatus, default=ReceivableStatus.PENDING)
    notes = models.TextField(blank=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(condition=Q(amount__gte=0), name="receivable_amount_non_negative"),
            models.CheckConstraint(condition=Q(due_date__gte=F("issued_at")), name="receivable_due_after_issue"),
            models.UniqueConstraint(
                fields=["service_agreement", "competence"],
                condition=Q(service_agreement__isnull=False, competence__isnull=False),
...
class PaymentQuerySet(models.QuerySet):
    ...
    def with_list_data(self):
        return self.select_related("receivable", "receivable__customer", "payment_method", "created_by", "voided_by")
